...
| M9 | Sofort-Sprachwechsel: DE/EN/SR-Chips steuern die ANTWORTSPRACHE des Lehrers (eigener System-Block `uiLanguageInstruction`, PREVOD dreht bei sr auf Deutsch). Satzweises Mitsprechen: der Avatar spricht jeden fertig gestreamten Satz sofort (`speakStream`/`endSpeakStream` in speech.ts — Chunk-Warteschlange für Browser- UND Server-TTS, Satzgrenzen-Erkennung mit Abkürzungsschutz in ChatPanel) | (Session 2) |
