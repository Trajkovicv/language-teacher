...
| M11 | Nutzungs-Statistik pro Profil (`client/src/lib/usage.ts`, localStorage): Übungsminuten (nur sichtbare Zeit), Sitzungen, Nachrichten, aktive Tage, erste Sitzung, Serie. Kompakte Zusammenfassung geht als `usage`-System-Block an `/api/chat`; Lehrer-Prompt gibt auf Nachfrage („wie lange/oft geübt", „Fortschritt", „Report") eine freundliche Übersicht (Zahlen + Lern-Gedächtnis). Sidebar-Stats: Minuten/Nachrichten/Serie pro Profil | (Session 2) |
| M10 | Live-Sprechsteuerung: Tempo 1×/1,5×/2× wirkt SOFORT (Azure via `audio.playbackRate`, nicht mehr SSML — deshalb Cache-Key ohne Tempo); Pause/Weiter + „Von vorn" während des Redens (Steuerknöpfe in der VoiceBar, `pausedRef` trennt Nutzer- von System-Pause). Zwei Lernprofile **Vuk** (fließend DE/EN/SR → Englisch verfeinern C1→C2) und **Andrijana** (perfektes SR → Deutsch lernen): Umschalter in der Sidebar, `client/src/lib/users.ts` + Server `learnerInstruction`/`learnerMemoryLine` in prompts.ts; Gedächtnis/Chat/Entwürfe pro User+Charakter getrennt (`lt-memory-<user>-<char>`), Lehrer spricht mit Namen an und fördert die Zielsprache | (Session 2) |
